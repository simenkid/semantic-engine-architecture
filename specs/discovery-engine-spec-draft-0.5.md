# Discovery Engine Specification – Draft 0.5 (Refined)

> **Document ID:** discovery-engine-spec-draft-0.5  
> **Status:** Draft (v0.5, refined core)  
...
The following elements are intended to remain stable once DE v0.5 is adopted:

1) **SD vocabulary alignment**
- SD phases MUST use the SD canonical names/spelling (Emergence, Divergence, Resonance, Distillation, Stabilization, Crystallization, Rest).
- Mismatch classes MUST use SD canonical IDs: M1_MotionExcess, M2_PrematureClosure, M3_OscillatoryNonConvergence, M4_SuppressedFlow.

2) **Explicit absence for RD dimensions**
- RD MUST represent missing/sparse dimensions via `availability: none|partial|full` (never by omitting fields).

3) **Metadata naming**
- RD generation metadata MUST use `generation_metadata` (not alternate keys), and SHOULD include at least `generator`, `generator_version`, and `notes`.

4) **Mismatch inference accounting**
- If mismatch inference is disabled, `data_quality.mismatch_inferred_rate` MUST be 0.0.
- If mismatch inference is enabled, `data_quality` MUST report mismatch coverage and inferred-rate.
...
### 0.1 ASCII Overview (Informative)
...
High-level pipeline:
  ...
  [SD Logs]
     |
     v
  (1) Flow Reconstruction  -- preserves order + labels (observed|inferred)
     |
     v
  (2) Functional Tagging   -- segments into Tagging Units (primary Functional Tag)
     |
     v
  (3) Aggregation over data_scope
     |
     v
  [RD]  -- phase_* + mismatch_* + tension_* + functional_* + data_quality + generation_metadata

Note: RD dimensions are always present; missing/sparse dimensions are represented via:
  availability: none|partial|full
```
...
| Mismatch Class | SD core imbalance class: M1 Motion Excess, M2 Premature Closure, M3 Oscillatory Non-convergence, M4 Suppressed Flow. |
...
- uses explicit SD annotations (phases/mismatch classes/tension snapshots) when available,
- MAY infer missing phases/mismatch classes, but MUST mark them as inferred.
...
- If DE infers `sd_phase` or `mismatch_class`, it MUST record label origin: `observed | inferred`.
...
- mismatch statistics (observed/inferred) when mismatch labels are available,
- tension signatures (if present),
- functional tag distributions and transitions (if tagging enabled),
and emits one or more RD artifacts.

`tension_signature` denotes a descriptive statistical summary of observed tension patterns derived from SD logs.
It represents characteristic distributions, frequencies, and tendencies of tension, and MUST NOT be interpreted as a user profile, preference model, or any Self Model enrichment construct.


Normative requirements:
- Each RD MUST include an explicit `data_scope`.
- For any RD dimension that is not computed or is too sparse (e.g., tension), DE MUST represent explicit absence by emitting the field with `availability: none|partial|full`, and reflecting coverage/inference in `data_quality` (see §6.2).
- DE MUST not include raw verbatim mind-pop text inside RD by default (privacy-by-default).

### 3.4 Relationship to Self Model Enrichment (Informative)

DE is intentionally *Self-Model-agnostic*. Its primary output is the **Rhythm Descriptor (RD)**, which is a portable descriptive artifact derived from SD Logs.

A downstream system (e.g., a compiler / packer layer) MAY use RD as an input to produce or update a user's **Self Model** and its **Enrich** layer:

- **Enrich.Profile**: long-term preferences, values, quality criteria, and interaction styles inferred or distilled from patterns captured in RD.
- **Enrich.Extension**: context- or task-scoped lenses, principles, constraints, and evaluation criteria constructed from subsets of RD under a declared scope.

Normative boundary:
- DE MUST NOT require knowledge of any specific Self Model schema.
- DE MUST produce RD with explicit `data_scope` and `data_quality` so downstream enrichment can remain traceable and auditable.
- Any mapping from RD → Self Model Enrich is outside DE's scope and MUST be specified by the downstream layer.
...
- `mismatch_class` (one of SD core mismatch classes: M1..M4)
- `tension_snapshot`


### 4.3 Label Origin

If `sd_phase` / `mismatch_class` are present, DE SHOULD preserve them as `observed`.
...
```yaml
mismatch_class:
  value: "M1_MotionExcess"
  origin: "inferred"    # or "observed"
  confidence: 0.74      # optional
```
...
- how mismatch classes appear and recover (when available),
- how tensions correlate with phases/mismatch classes (when available),
...
RD MAY additionally include **pattern metrics** that capture longer-horizon structure beyond phase/mismatch_class/tags,
including `thread_metrics`, `branch_metrics`, and `recovery_patterns` (see Appendix B).

#### RD Shape (ASCII, Informative)

```text
RD (Rhythm Descriptor) – minimal shape:
...
| mismatch_statistics {availability, items{M1..M4}}                 |
| tension_signature     {availability, ...}                          |
| functional_distribution {availability, ...}                      |
| functional_transitions  {availability, adjacency, matrix}        |
| data_quality (coverage + inferred-rate)                          |
| generation_metadata (generator, version, notes, ...)             |
+------------------------------------------------------------------+
```
...
- `mismatch_statistics` – may be empty if unavailable, but field must exist with explicit availability.
- `tension_signature` – same rule: explicit availability.
...
rd_descriptor:
  ...
  mismatch_statistics:
    ...
    items:
      M1_MotionExcess: { freq: 0.12 }
      M2_PrematureClosure: { freq: 0.05 }
      M3_OscillatoryNonConvergence: { freq: 0.00 }
      M4_SuppressedFlow: { freq: 0.00 }
  ...
  data_quality:
    ...
    mismatch_label_coverage: 0.15
    mismatch_inferred_rate: 0.00        # set >0 only if mismatch inference enabled
  ...
  generation_metadata:
    ...
    notes: "Phases inferred when missing; mismatch inference disabled."
```
...
- MUST expose inferred vs observed coverage in `data_quality` at least for phases; if mismatch inference is enabled, it MUST also expose mismatch inferred-rate/coverage.
...
### 7.2 Minimal Conformance Set (RD-Generating)
...
`tension_signature`:
  - A descriptive statistical aggregation capturing the characteristic shape of observed tension dynamics (e.g., distribution, recurrence, transitions).
  - This field is analytical and observational, not prescriptive or identity-bearing.

For early drafts, the DE MAY emit any of the following as `availability: none` when not computed:
- mismatch_statistics
- tension_signature
- functional_distribution / functional_transitions

In all cases, explicit availability MUST be stated and reflected in `data_quality`.
...
rd_descriptor:
  ...
  mismatch_statistics:
    availability: "none"   # none|partial|full
    items: {}
  ...
  functional_distribution:
    availability: "full"   # none|partial|full
    OBSERVE: 0
    FRAME: 0.5
    ABSTRACT: 0
    STRUCTURE: 0
  ...
  data_quality:
    ...
    mismatch_label_coverage: 0.0
    mismatch_inferred_rate: 0.0
    tension_coverage: 0
  ...
  generation_metadata:
    generated_at: "2025-12-12T00:00:00Z"
    update_mode: "batch"            # batch|incremental
    scope_revision: null            # optional; recommended when incremental
    generator: "de"
    generator_version: "0.3.0"
    notes: "Phases inferred when missing; mismatch inference disabled."
```
...
These fields are designed to be useful even when tensions/mismatch classes are sparse.
...
`recovery_patterns` captures **recurring exit motifs** from an undesired state (mismatch_class or phase),
...
recovery_patterns:
  ...
  items:
    - trigger:
        mismatch_class: "M1_MotionExcess"     # optional
        phase: null                     # optional
      recovery:
    ...
    - trigger:
        mismatch_class: "M3_OscillatoryNonConvergence"
      recovery:
